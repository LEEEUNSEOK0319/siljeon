...
interface ApiKeyModalProps {
  isOpen: boolean;
  onClose: () => void;
  // 저장 직후 목록 새로고침이 필요하면 부모에서 이 콜백을 내려주세요.
...
export function ApiKeyModal({ isOpen, onClose, onSave, editingKey }: ApiKeyModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 2) Dooray 드라이브 연결 시도
      const conn = await fetch('http://localhost:8090/api/dooray/driveConnect', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      // 선택: 응답이 배열(drives)이지만 비어 있으면 실패로 볼 수도 있음
      let ok = conn.ok;
      try {
        const payload = await conn.json(); // drives 배열일 수 있음
        if (Array.isArray(payload) && payload.length === 0) {
          ok = false;
        }
      } catch {
        // 응답이 JSON이 아닐 수도 있으니 무시
      }
        ...
        <div className="p-6 space-y-6">
          <div className="text-center">
            <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
              Dooray 드라이브에서 파일을 검색하려면 API 키가 필요합니다. 키는 안전하게 저장됩니다.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            {validationStatus === 'invalid' && (
              <p className="text-sm text-red-600 dark:text-red-400 flex items-center space-x-1">
                ...
                <span>연결에 실패했습니다. 키를 확인해 주세요.</span>
              </p>
            )}
            {validationStatus === 'valid' && (
